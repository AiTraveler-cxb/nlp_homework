...
def run(question: str, max_steps: int = 10, messages: list = None) -> Generator[dict, None, None]:
    ...
    支持传入已有历史消息，实现多轮对话；如果未传入 history，则按单轮问题运行。
...
class Conversation:
    """简单会话类，保存 Function Calling 版对话历史并支持多轮问答。"""
    def __init__(self, system_prompt: str = FC_SYSTEM_PROMPT, max_steps: int = 10):
        self.messages = [{"role": "system", "content": system_prompt}]
        self.max_steps = max_steps

    def ask(self, question: str, max_steps: int = None):
        """追加当前问题并执行一次 ReAct 循环。"""
        if max_steps is None:
            max_steps = self.max_steps
        return run(question=question, max_steps=max_steps, messages=self.messages)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interactive", default=True, action="store_true", help="进入交互式多轮对话模式")
    args = parser.parse_args()

    if args.interactive:
        conv = Conversation(max_steps=args.max_steps)
