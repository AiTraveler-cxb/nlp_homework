...
def run(question: str = None, max_steps: int = 10, verbose: bool = True, messages: list = None) -> Generator[dict, None, None]:
    """
    ...
    if messages is None:
        ...
        # 如果提供了 messages，并提供了 question，则把本轮用户问题追加到历史里
    ...
    for step in range(1, max_steps + 1):
        ...
        print(f"\n[LLM Output Step {step}]:\n{llm_output[:500]}{'...' if len(llm_output) > 500 else ''}") if verbose else None
...
class Conversation:
    """简单会话类，保存对话历史并支持多轮问答。"""
    def __init__(self, system_prompt: str = SYSTEM_PROMPT, max_steps: int = 10, verbose: bool = True):
        self.messages = [{"role": "system", "content": system_prompt}]
        self.max_steps = max_steps
        self.verbose = verbose

    def ask(self, question: str, max_steps: int = None) -> Generator[dict, None, None]:
        """向会话中追加用户问题并运行 ReAct 循环，yield 每一步结果。"""
        if max_steps is None:
            max_steps = self.max_steps
        # 将当前问题追加到历史并调用 run()，run 会在结束时把工具调用结果追加回 history
        return run(question=question, max_steps=max_steps, verbose=self.verbose, messages=self.messages)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interactive", action="store_true", help="进入交互式多轮对话模式")
    args = parser.parse_args()
    if args.interactive:
        conv = Conversation(max_steps=args.max_steps, verbose=True)
        print("进入交互式多轮对话，输入 'exit' 或 'quit' 退出。")
        while True:
            try:
                q = input("问: ")
            ...
            start = time.time()
            for step_data in conv.ask(q, max_steps=args.max_steps):
                stype = step_data["type"]
                if stype == "action":
                    print(f"\n[Step {step_data['step']}]")
                    print(_c("thought", f"🧠 Thought: {step_data['thought']}"))
